...
<div class="min-h-screen bg-adventure-beige pb-16">
    ...
    {{-- ░░ HERO — fixed to 420px tall so it's actually visible ░░ --}}
    <section class="relative min-h-[280px] h-[360px] overflow-hidden">
        ...
        <div class="relative z-10 mx-auto flex h-full w-full max-w-7xl items-end px-4 pb-10 md:px-6">
            <div>
                ...
                <p class="my-5 text-base text-white/70">Explore handpicked adventures across Nepal</p>
            </div>
        </div>
    </section>
    ...
    {{-- ░░ MAIN LAYOUT ░░ --}}
    {{--
        The sidebar sticky requires:
        1. NO overflow:hidden/auto on any ancestor (check layouts.app)
        2. items-start on the flex row
        3. self-start on the aside
        As a fallback, we use position:sticky via inline style so Tailwind purge can't strip it
    --}}
    <div class="mx-auto mt-8 w-full max-w-7xl px-4 md:px-6">
        <div class="flex items-start gap-8">
            ...
            {{-- ░░ LEFT SIDEBAR ░░ --}}
            <aside
                class="hidden w-64 shrink-0 self-start lg:block"
                style="position: sticky; top: 80px; align-self: flex-start;">

                <div class="rounded-2xl border border-[#1a3a2a]/10 bg-white p-5 shadow-sm">

                    {{-- Categories --}}
                    <p class="mb-3 text-xs font-semibold uppercase tracking-widest text-[#1a3a2a]/40">Categories</p>
                    ...
                    {{-- Difficulty --}}
                    <label class="mb-2 block text-xs font-semibold uppercase tracking-widest text-[#1a3a2a]/40">Difficulty</label>
                    <select
                    ...
                    {{-- Duration --}}
                    <label class="mb-2 block text-xs font-semibold uppercase tracking-widest text-[#1a3a2a]/40">Duration</label>
                    <select
            ...
            <main class="min-w-0 flex-1">
                ...
                <div class="mb-4 flex flex-wrap gap-2 lg:hidden">
                    ...
                    <button type="button" wire:click="resetFilters"
                        class="rounded-lg border border-[#c8860a] px-3 py-2 text-sm font-semibold text-[#c8860a]">Reset</button>
                </div>
                ...
                <div class="mb-5 flex flex-wrap items-center gap-2">
                    ...
                    @endif
                    <button type="button" wire:click="resetFilters"
                        class="text-xs font-semibold text-[#c8860a] underline underline-offset-2">Reset all</button>
                </div>
                @endif
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-3">
                    ...
                    ? $trip->cover_image
                    : \Illuminate\Support\Facades\Storage::url($trip->cover_image ?? '');
                    ...
                    <article class="group overflow-hidden rounded-3xl border border-[#1a3a2a]/10 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-md">
                        ...
                        <div class="p-5">
                            <h3 class="font-display text-lg font-bold leading-snug text-[#1a3a2a]">{{ $trip->title }}</h3>
                            <div class="mt-2 flex flex-wrap items-center gap-2 text-sm text-[#1a3a2a]/70">
                                ...
                                <span class="rounded-full px-2.5 py-0.5 text-xs font-semibold {{ $difficultyColor }}">
                                    {{ ucfirst($trip->difficulty) }}
                                </span>
                            </div>
                            <p class="mt-1.5 text-xs text-[#1a3a2a]/50">Best season: {{ $trip->best_season }}</p>
                            <p class="mt-3 text-lg font-bold text-[#c8860a]">
                                @if ($trip->starting_price)
                                From ${{ number_format($trip->starting_price, 0) }}
                                @else
                                <span class="text-sm font-medium text-[#1a3a2a]/40">Price on request</span>
                                @endif
                            </p>
                            <div class="mt-4 flex w-full gap-2">
                                <a href="{{ route('trips.show', $trip->slug) }}" class="tn-btn-secondary flex-1 justify-center">View Trip</a>
                                <button type="button"
                                    x-on:click="Livewire.dispatch('open-booking-popup', { tripId: {{ $trip->id }} })"
                                    class="tn-btn-primary flex-1 justify-center">Book Now</button>
                            </div>
                        </div>
                    </article>
                    @endforeach
