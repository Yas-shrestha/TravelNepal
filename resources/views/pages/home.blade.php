...
@section('content')
    ...
    {{-- Hero (full viewport from top:0; sits behind fixed navbar — padding only on inner content) --}}
    <section class="relative flex min-h-[100svh] w-full items-center justify-center overflow-hidden pb-24 pt-0">
        ...
        <div class="relative z-10 mx-auto max-w-5xl px-4 pt-[calc(72px+2rem)] text-center md:px-6 md:pt-[calc(72px+3rem)]">
            ...
            <div class="mx-auto mt-10 flex w-full max-w-xl flex-col justify-center gap-4 sm:max-w-none sm:flex-row sm:gap-6">
                <a href="{{ route('trips.index') }}" class="tn-btn-primary justify-center px-8 py-3 transition hover:brightness-110">Explore Trips</a>
        ...
        <div class="mx-auto w-full max-w-7xl px-4 md:px-6">
            ...
            <div class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
                @foreach ($featuredTrips as $trip)
                    ...
                    @endphp
                    <article class="overflow-hidden rounded-3xl border border-[#1a3a2a]/10 bg-white shadow-sm transition duration-300 hover:-translate-y-1">
                        <div class="relative">
                        ...
                        <div class="p-6">
                            <h3 class="font-display text-2xl font-bold text-[#1a3a2a]">{{ $trip->title }}</h3>
                            ...
                            <div class="mt-5 flex w-full flex-col gap-3 sm:flex-row">
                                ...
                                <button type="button" x-on:click="Livewire.dispatch('open-booking-popup', { tripId: {{ $trip->id }} })" class="tn-btn-primary w-full justify-center sm:w-auto px-6 py-3 transition hover:brightness-110">Book Now</button>
                            </div>
                        </div>
                    </article>
                @endforeach
            ...
            <div class="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
                @foreach ([
                    ...
                    <div class="rounded-2xl border border-white/10 bg-white/5 p-6 transition duration-300 hover:-translate-y-1">
                        @if ($item['icon'] === 'shield')
                            <svg class="h-10 w-10 text-[#c8860a]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" /></svg>
                        @elseif ($item['icon'] === 'users')
                            <svg class="h-10 w-10 text-[#c8860a]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.813-2.513M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" /></svg>
                        @elseif ($item['icon'] === 'wrench')
                            <svg class="h-10 w-10 text-[#c8860a]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M11.42 15.17 17.25 21A2.652 2.652 0 0 0 21 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655-5.653a2.548 2.548 0 0 0-3.586 0L2.5 8.5" /></svg>
                        @else
        ...
        <section
            ...
            x-data="{
                ...
                updatePerPage() {
                    ...
                    this.perPage = w >= 1024 ? 3 : (w >= 768 ? 2 : 1);
                },
                maxIdx() {
                ...
                measure() {
                    ...
                    this.cardWidth = pp > 0 ? (usable - this.gap * (pp - 1)) / pp : usable;
                },
                slidePx() {
                ...
                dotIndexes() {
                    return Array.from({ length: this.maxIdx() + 1 }, (_, index) => index);
                },
                startAutoplay() {
            ...
            <div class="mx-auto w-full max-w-7xl px-4 md:px-6">
                ...
                <div class="relative mx-auto flex max-w-6xl items-center gap-2 md:gap-4">
                    ...
                    <div class="min-w-0 flex-1 overflow-hidden" x-ref="viewport">
                        <div
                            class="flex gap-6 transition-[transform_0.4s_ease]"
                            ...
                            @foreach ($testimonials as $testimonial)
                                ...
                                >
                                    <div class="flex flex-row items-start gap-4">
                                        <img src="{{ $avatarUrl }}" alt="{{ $testimonial->name }}" class="h-16 w-16 shrink-0 rounded-full object-cover" width="64" height="64" />
                                        <div class="min-w-0 flex-1 text-left">
                                            <h3 class="break-words font-display text-lg font-semibold text-adventure-green">{{ $testimonial->name }}</h3>
                                            <p class="break-words font-sans text-sm text-gray-500">{{ $testimonial->location }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <p class="mt-4 flex-1 break-words font-sans text-base italic leading-relaxed text-gray-600">"{{ $testimonial->quote }}"</p>
                                    <div class="mt-auto pt-6">
                                        @if ($testimonial->trip)
                                            <a href="{{ route('trips.show', $testimonial->trip->slug) }}" class="break-words font-sans text-sm font-medium text-adventure-ochre transition hover:brightness-110">{{ $testimonial->trip->title }}</a>
                                        @endif
                                    </div>
                                </article>
                            @endforeach
