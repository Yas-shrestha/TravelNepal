...
@section('content')
    @php
        ...
        $galleryUrls = $trip->images
            ->map(fn ($image) => Illuminate\Support\Str::startsWith($image->image_path, ['http://', 'https://']) ? $image->image_path : Storage::url($image->image_path))
            ->values()
            ->all();
    ...
    <div x-data="{ pastHero: false }" x-init="window.addEventListener('scroll', () => { pastHero = window.scrollY > 300 }, { passive: true })">
        <section class="relative min-h-[320px] h-[420px] overflow-hidden sm:min-h-0">
            <img src="{{ $imageUrl }}" alt="{{ $trip->title }}" class="absolute inset-0 h-full w-full object-cover" />
                ...
                <div class="lg:sticky lg:top-24">
                    <div class="rounded-3xl border-t-4 border-[#1a3a2a] bg-white p-6 shadow-lg">
                        ...
                        <div class="mt-6 space-y-4 font-sans text-sm">
                            ...
                            <p><strong>Difficulty:</strong> <span class="rounded-full px-2 py-1 text-xs {{ $difficultyColor }}">{{ ucfirst($trip->difficulty) }}</span></p>
        ...
        <button
            ...
            class="fixed bottom-28 right-4 z-40 max-w-[calc(100vw-2rem)] truncate rounded-full bg-[#c8860a] px-5 py-3 text-center font-sans text-xs font-semibold uppercase tracking-wide text-white shadow-lg transition hover:brightness-110 sm:text-sm md:bottom-6 md:right-5 md:max-w-none"
            style="display: none;"
        >
